    ...
    for motherboard_item in motherboard_items:
        ...
        for motherboard_overview in motherboard_overviews:
            ...
            if motherboard_techspecs is None:
                print("Error: msi motherboard techspec not found")
                exit(0)
                continue
...
def parse_motherboard_techspec_page(content, motherboard_overview):
    ...
    if motherboard_techspec is None:
        print("Error: msi motherboard techspec name not found")
        exit(0)
    ...
    if len(motherboard_techspecs_rows) == 0:
        print("Error: msi motherboard techspec rows not found")
...
def parse_motherboard_techspec_rows(content, motherboard_overview):
    ...
    selectors = [
        '#spec-basic table tr',
        '#spec-detail .table-specifications div.row',
    ]
    motherboard_techspecs = []
    for index, selector in enumerate(selectors):
        items = soup.select(selector)
        if len(items) == 0:
            print("Error: msi motherboard techspec rows not found (selector: %s)" % selector)
            # exit()
            continue
        if index == 0:
...
def parse_motherboard_techspec_name(content, motherboard_overview):
    ...
    for selector in selectors:            
        ...
        if name is None:
            print("Error: msi motherboard techspec name not found (selector: %s)" % selector)
            # exit()
            continue
        name = name.text
        motherboard_techspec = MotherboardTechSpec(
